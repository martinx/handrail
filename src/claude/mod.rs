...
pub const STATE: &str = "handrail/state.json";
...
/// True when an installed file and what would be written now differ only in a way that
/// carries no policy: versions up to 0.2.1 put their version number in the CLAUDE.md
/// block's title. Without this, everyone upgrading would be told to re-apply once for a
/// changed title alone.
pub fn equivalent(path: &str, installed: &[u8], desired: &[u8]) -> bool {
    ...
    let norm = |b: &[u8]| -> String {
        String::from_utf8_lossy(b)
            .lines()
            .map(|l| {
                if l.starts_with("# Local policy (Handrail") {
...
fn render_block(packs: &[&Pack], local_rules: &[LocalRule], version: &str) -> String {
    ...
    // No version number here: it would make every upgrade rewrite this file even when no
    // pack changed. The version is recorded in the state file.
    let _ = version;
    s.push_str("# Local policy (Handrail)\n\n");
    s.push_str(
